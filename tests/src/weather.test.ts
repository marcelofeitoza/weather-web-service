...
describe("Weather API performance", () => {
  ...
  it(`GET /weather/${DEFAULT_CITY} during high load should still be less than ${MEAN_LIMIT}ms`, async () => {
    ...
    for (let i = 0; i < REQUEST_TIMES + 1; i++) {
      const start = new Date().getTime();
      const response = await request.get(
        `/weather/${encodeURIComponent(DEFAULT_CITY)}`,
      );
    ...
    const allRequestsSuccessful = requestTimes
      .slice(1)
      .every((request) => request.status === SUCCESS_STATUS);
    const meanRequestTime =
      requestTimes.reduce((acc, request) => acc + request.time, 0) /
      REQUEST_TIMES;
